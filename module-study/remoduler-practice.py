...
# # ================================= 单次匹配 ================================
# match 方法,默认必须从0处匹配上，或指定位置开始匹配,第一个匹配不上就返回none
...
regex1 = re.findall('b(\\w+)', s)   # 单个分组，findall就返回组1的字符串
for i in regex1:
    print(type(i), i)

regex2 = re.findall('(b)(\\w+)', s)  # 这里是多个分组，就返回字符串元组
for i in regex2:
    print(type(i), i)
    print(i[0], i[1])           # 获取分组内容,即通过元组的下标取值,这里似乎没法用 i.groups 方法

# finditer
regex1 = re.finditer('(b)(\\w+)', s)
for i in regex1:
    print(type(i), i, i.groups(), i.group(2))  # 获取分组内容 finditer 方法可以使用 groups
